...
text = load_data("Three Little Pigs.txt")
...
seqLen = 20 #toggle between different values
sequence = list()
...
data_to_save = "\n".join(sequence)
save_data(data_to_save, "../Text Files and Dictionary/TLP sequence.txt")
...
sequenceData = load_data("TLP sequence.txt")
...
model.fit(x, y, epochs= 140, verbose=2)

model.save("learnedModel.h5")
dump(dictionary, open("dictionary.pkl", "wb"))

model = load_model("learnedModel.h5")
dictionary = load(open("dictionary.pkl", "rb"))
...
print(generateSeq(model, 20, 30," Once upon a time"))
print(generateSeq(model, 20, 30," There was an"))
print(generateSeq(model, 20, 30," And he huffed and puffed"))
